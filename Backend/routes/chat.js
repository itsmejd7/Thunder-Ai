...
router.get("/thread/:threadId", async (req, res) => {
  ...
  try {
    if (typeof threadId !== 'string' || threadId.length > 200) return res.status(400).json({ error: "Invalid threadId" });
...
router.post("/chat", async (req, res) => {
  ...
  try {
    ...
    try {
      const userId = req.user.userId;
      let thread = await Thread.findOne({ threadId, user: userId });
      if (!thread) thread = new Thread({ threadId, title: message, messages: [], user: userId });
